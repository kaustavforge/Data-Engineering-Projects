...
def render(frame_index):
    ...
    hd.ellipse((110, -250, 650, 340), fill=(45, 210, 235, 22))
    hd.ellipse((950, -190, 1550, 380), fill=(102, 120, 255, 25))
    haze = haze.filter(ImageFilter.GaussianBlur(85))
    ...
    if visible == len("WALMART PROJECT"):
        ...
        line_width = int(360 + 700 * min(1, max(0, (frame_index - 28) / 6)))
        center = WIDTH // 2
        d = ImageDraw.Draw(base)
        d.rounded_rectangle((center-line_width//2, 224, center+line_width//2, 230), radius=3,
                            fill=(int(45+50*pulse), int(190+40*pulse), 255))
        subtitle = "DATA ENGINEERING  •  LAKEHOUSE  •  ORCHESTRATION"
        subtitle_box = d.textbbox((0, 0), subtitle, font=sub_font)
        subtitle_width = subtitle_box[2] - subtitle_box[0]
        subtitle_x = (WIDTH - subtitle_width) // 2
        d.text((subtitle_x, 266), subtitle, font=sub_font, fill=(192, 208, 231))
        dot_x = center - line_width//2 + int(((frame_index - 28) % 12) / 11 * line_width)
        d.ellipse((dot_x-7, 220, dot_x+7, 234), fill=white)

    return base

frames = [render(i) for i in range(FRAMES)]
# A shared palette keeps the looping banner small for a repository README.
palette_source = Image.new("RGB", (800, 180))
palette_source.paste(frames[0].resize((400, 180)), (0, 0))
palette_source.paste(frames[-1].resize((400, 180)), (400, 0))
